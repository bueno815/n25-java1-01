...
public class Baralho {
	...
	private Random rnd; // Gerar aleatorio

	public Baralho() {

	rnd=new Random();
	// Criar o espaço das Cartas
	cartas=new Carta[Carta.NAIPES.length*Carta.NUMEROS.length];cartaRetirada=new boolean[cartas.length];
	// Populando as cartas e cartaRetirada
	for(

	int i = 0;i<cartas.length;i++)
	{
...
public Carta getCartaSorteada() {
	...
	return null; //se nao tiver mais carta retorna null

	}
public int getNumCartasDisp() {
	int ret =0;
	for(boolean it: cartaRetirada) {
		if (it) { //nao foi retirada
			ret++;
		}
